	...
	<div class="controlItem " >
		<div class="editControl"  v-if="show==false">
			<div class="editControl-div">
			   <el-input placeholder="请输入题目" v-model="title" >
			    <template slot="prepend" >单选题目</template>
			   </el-input>
			</div>
			<div class="editControl-div">
			   <el-input placeholder="请输入备注"  v-model="beizhu">
			    <template slot="prepend">单选备注</template>
			   </el-input>
			</div>
			<div class="editControl-div">
		...
		<div class="showControl" v-if="show"  >
			...
			<el-radio-group v-model="radio2">
			    <el-radio :label="index" v-for="(obj,index) in numsource">{{obj.text}}</el-radio>
			 </el-radio-group>
        	...
        	creatIt(){
        		for(let i=0;i<this.numsource.length;i++){
                 ...
                 return false;
        			...
        			this.show=true;
                  return  this.getData();
                 /* let   getForData={"content":"123","naireName":"测试数据","img":["img1","img2"]}
                    console.log("1weqweqeqwq",getdata1,getForData)
                    dataget("post",getdata1," ","/qnaire/Quesnaire/create",this.getTableSuccess);*/ 
